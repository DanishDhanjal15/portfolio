...
`;function r(n,e=!1){if(n==="")return;const t=document.createElement("div");t.className=e?"command-line":"command-output",t.innerHTML=n,m.appendChild(t),u.scrollTop=u.scrollHeight}function b(n){const e=n.trim().toLowerCase();if(r(`<span id="prompt">danishdhanjal@portfolio:~$</span> <span class="command-text">${e}</span>`,!0),e!=="")if(l[e]){const t=l[e]();t&&(e==="clear"?l.clear():r(t))}else r(`<span class="error-msg">Command not found: ${e}. Type 'help' for available commands.</span>`)}const f=document.getElementById("custom-cursor"),y=document.getElementById("cursor-follower");document.addEventListener("mousemove",n=>{const e=n.clientX,t=n.clientY;f.style.left=`${e}px`,f.style.top=`${t}px`,y.style.left=`${e}px`,y.style.top=`${t}px`});document.addEventListener("mouseover",n=>{const e=n.target;(e.tagName==="A"||e.tagName==="BUTTON"||e.tagName==="INPUT"||e.tagName==="TEXTAREA"||e.classList.contains("accent")||e.closest(".social-btn"))&&document.body.classList.add("cursor-hover")});document.addEventListener("mouseout",n=>{const e=n.target;(e.tagName==="A"||e.tagName==="BUTTON"||e.tagName==="INPUT"||e.tagName==="TEXTAREA"||e.classList.contains("accent")||e.closest(".social-btn"))&&document.body.classList.remove("cursor-hover")});c.addEventListener("keydown",n=>{if(n.key==="Enter"){const e=c.value;b(e),c.value=""}});function g(){p.classList.add("hidden"),c.focus()}v.addEventListener("click",g);i.addEventListener("submit",async n=>{n.preventDefault();const e=new FormData(i),t=e.get("name")||"Someone",o=i.querySelector(".btn-primary"),s=o.innerHTML;o.innerHTML="<span>⏳</span> Sending...",o.disabled=!0;try{if((await fetch(i.action,{method:"POST",body:e,headers:{Accept:"application/json"}})).ok)g(),r(`<span class="success-msg">[SYSTEM] ✓ Message received from ${t}!</span>`),r('<span class="system-msg">Thank you for your message. I will get back to you shortly.</span>'),i.reset();else throw new Error("Form submission failed")}catch{r('<span class="error-msg">[ERROR] Failed to send message. Please try again or email directly.</span>')}finally{o.innerHTML=s,o.disabled=!1}});document.addEventListener("click",()=>{p.classList.contains("hidden")&&c.focus()});async function E(){c.style.display="none";const n=document.querySelectorAll("#prompt");n.forEach(t=>t.style.display="none");const e=[{text:"Initializing system...",color:"system-msg"},{text:"Loading kernel modules...",color:"system-msg"},{text:"Establishing secure connection...",color:"system-msg"},{text:"Access granted.",color:"success-msg"}];for(const t of e)await new Promise(o=>setTimeout(o,400)),r(`<span class="${t.color}">${t.text}</span>`);await new Promise(t=>setTimeout(t,600)),m.innerHTML="",h.innerHTML=T,await new Promise(t=>setTimeout(t,1e3)),c.style.display="block",n.forEach(t=>t.style.display="inline"),c.focus()}E();
